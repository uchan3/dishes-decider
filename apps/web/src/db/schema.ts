...
export interface OutboxRow {
  ...
  created_at: string;
  /**
   * 何度送っても直らないと判明した時刻（列が無い・制約違反など）。
   *
   * これが入った行は**送信対象から外れる**。1 件の恒久エラーがキューを塞いで、
   * 他の書き戻し（レシピ編集・買い物リスト）まで巻き添えにするのを防ぐため。
   * インデックスは張らない（件数が少なく、メモリで分ければ足りる）。
   */
  failed_at?: string;
  /** 失敗の理由（設定画面に出して、何が送れていないかを分かるようにする）。 */
  error?: string;
}
