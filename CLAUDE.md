...
- **ビルド変数（Settings → Build → Build variables and secrets）**: `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY`（ビルド時に焼き込まれる）＋ `NODE_VERSION=22`
  - `NODE_VERSION` は 2026-08-18 に明示設定した（それまでは未設定＝ビルドイメージ既定で動いていた）。**supabase-js の realtime が Node 22+ のネイティブ WebSocket を要求する**ため、CI（`.github/workflows/ci.yml` も 22）と揃えている
  - 変更は**次のビルドから**適用される（既存デプロイには遡及しない）。すぐ反映したいときはビルドを Retry する
...
- ブラウザからはページ本文を取れない（CORS）ため PWA の取り込みは既定で**サーバー fetch 経路**。Bot 対策の厳しいサイトはここで失敗しうるので、失敗時は「本文を貼り付ける」か、端末で本文を取るショートカット経路に逃がす
- **Instagram はログイン必須でサーバーからは本文が読めない**（US-01 は Must なのに黙って失敗ジョブになっていた）。`needsPastedContent`（`deriveSource` の kind で判定・テスト有り）で**投げる前に**見分け、キャプションの貼り付け欄を出して `{url, content, contentKind:"text"}` で送る＝ショートカットと同じ入口を使う。**ショートカット経由でも Instagram は読めない**（Shortcuts アプリにログインセッションが無い）ので、Instagram は PWA の貼り付けが正規の導線
...
- `src/lib/planning.ts` — core (`generateMealPlan` / `aggregateShoppingList`) と Dexie を繋ぐ層。`generateWeek()` が献立を生成し Dexie に保存、`buildShoppingItems()` が買い物リストを集約。**週の「作り直す」は常に `generateWeek`**（現在の曜日テンプレで構造から再生成し、既存プランのロック済みスロットを slotId 一致で引き継ぐ）。スロット/食事の再抽選(US-05): `reshuffleSlot`(必ず別レシピ・代替無ければ現状維持) / `reshuffleMeal`（ロック維持）/ `toggleSlotLock`(US-06)。**手動編集(F-02-4)**: `clearSlot`(空にする) / `setSlotRecipe`(ライブラリから指定。**自動でロックする** — 自分で選んだ枠が次の作り直しで消えるのは意図と食い違うため) / `setMealSkipped`(外食・作らない。**スロットの中身は消さない**ので戻せば元どおり) / `setMealTemplate`(その日だけ構成を変える。残る枠は slotId 一致で中身を引き継ぎ、増えた枠だけ抽選。**作った記録がある日は変更しない** — 枠が減ると記録ごと消えるため)。外食の日は `buildShoppingItems` が丸ごと除外し、`generateWeek` も抽選しない（候補を無駄に使わない）。**手動の外食指定は作り直しでも引き継ぐ**（毎回付け直さなくてよい）。UI は `components/RecipePicker.tsx`（ライブラリと同じ `filterRecipes` を使うので材料名でも引ける）。共通ロジックは `reshuffleSlots`（対象外・ロック済みのレシピを除外して再抽選）。生成・再抽選とも `loadEligibleRecipes()` 経由でレシピを読み、**無効ソース(`is_enabled=false`)のレシピを除外**(US-03)
- `src/lib/reject.ts` — 却下理由の記録(F-02-3)。`reject_count` 列はあったのに誰も増やしておらず、**何度再抽選しても学習しなかった**。「気分じゃない」→ `meal_plans.excluded_recipe_ids`（その週だけ。翌週は戻る）/「最近食べた」→ `recipes.snoozed_until`（**作った記録にはしない** — よそで食べただけで `cook_count` が増えると novelty が実態とずれる）/「材料が面倒」→ `reject_count`（スコアの reject ペナルティ）/「もう出さないで」→ `is_excluded`。**訊くのは再抽選の後**（↻ は一番よく押すボタンなので、押すたびにダイアログが出ると邪魔になる）。どちらも `loadEligibleRecipes` / `reshuffleSlots` / `generateWeek` が候補から外す
...
- `src/lib/shopping.ts` — 買い物リストの永続化(US-09)。`syncShoppingList()` が集約結果を `shoppingItems` に保存し、`reconcileShoppingItems()`（純粋関数・テスト有り）が**既存項目の id とチェック状態を引き継ぐ**（同一性キーは `ingredient_id ?? name:正規化表示名`）。献立から消えた項目は削除、`is_manual` の項目は常に残す。**常備品も含めて保存し表示側でフィルタする**（トグルで行を作り直すとチェックが消えるため）。`setItemChecked` / `clearChecked` / `pantryIngredientIds`。**数量の手直し(F-03-1)**は `quantity_override` に分けて持ち、計算値 `quantity` は残す（`effectiveQuantity` が実効値。作り直しても手直しは引き継ぎ、「元に戻す」で計算値に帰れる。**献立が変わって必要量が増えても手直しを優先する** — 「家にある分を引いた」という意図のほうが確かなため）。端末間は項目単位マージでチェック状態と一緒に運ぶ。**内訳表示**は `source_recipe_ids` をレシピ名に引き直して出す（買い物中の「これ何に使う分だっけ」に答える）。手動追加(US-13)は `addManualItem`（売場は `classifyIngredient` で推定・`is_manual` を立てるので作り直しでも残る）/ `removeShoppingItem`。**献立が無い週でも追加できる**（先に買うものだけ登録しておける）
- 曜日ごとの献立構成(US-07): `lib/mealTemplates.ts`（プリセット6種＋月起点の週割り当て）＋ `lib/settings.ts`（Dexie の `settings` KVストア。schema **v2** で追加）。設定画面で曜日別にテンプレ選択。**設定は `lib/settingsSync.ts` で端末間同期する**（`user_settings.doc` に 1 ユーザー 1 行の jsonb。献立ドキュメントと同じ考え方で、サーバーは中身を読まない）。競合は**ドキュメント単位の LWW**（`settings_updated_at` キーが時計）。送信は outbox の `settingsDoc` 経由・受信は起動時の `pullSettings()` と Realtime。**受信時は相手の時計をそのまま採用する**（受け取った瞬間を自分の更新時刻にすると古い設定を送り返してしまう）。値の健全化は `applySettings` が `sanitize`/`normalizePlanningSettings` を通すので、壊れた値は生成ロジックに流れない。`eat_out`(空スロット) の日は `is_skipped` の Meal になり生成対象外
